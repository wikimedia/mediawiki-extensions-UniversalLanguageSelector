...
| `ext.uls.rewrite.languagesettings` | `LanguageSettingsEntrypoint.js`| Registers the "Language settings" quick action that opens the legacy ULS settings dialog. | Loaded by the host page so that the settings action is available out of the box. Depends only on `ext.uls.rewrite.entrypoints`; the legacy settings dialog is loaded lazily via `mw.loader.using` when the action is invoked. |
